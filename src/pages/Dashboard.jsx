...
import { FaWallet, FaChartLine, FaMoneyBillWave, FaExchangeAlt, FaArrowUp, FaArrowDown, FaEye, FaEyeSlash, FaCopy } from 'react-icons/fa';
          ...
          <div className="bg-slate-800 rounded-2xl p-6 shadow-xl">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <button 
                ...
                className="p-4 bg-gradient-to-r from-blue-600 to-blue-700 rounded-xl hover:from-blue-700 hover:to-blue-800 transition"
              >
                <FaArrowUp className="mx-auto mb-2 text-white" />
                <span className="text-white font-semibold">Deposit</span>
              </button>
              <button 
                onClick={() => navigate('/withdraw')}
                className="p-4 bg-gradient-to-r from-orange-600 to-orange-700 rounded-xl hover:from-orange-700 hover:to-orange-800 transition"
              >
                <FaArrowDown className="mx-auto mb-2 text-white" />
            ...
            <div className="bg-slate-700/50 rounded-xl p-4">
              ...
              <div className="flex items-center justify-between">
                <code className="text-xs text-white truncate">{walletData.walletAddress || 'No address'}</code>
                <button onClick={copyAddress} className="p-2 bg-slate-600 rounded-lg hover:bg-slate-500 transition">
              ...
              {activeInvestments.map((investment, index) => (
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <h3 className="font-bold text-white">{investment.plan || investment.planName}</h3>
                      <p className="text-xs text-slate-400">Started: {investment.startDate ? new Date(investment.startDate).toLocaleDateString() : 'N/A'}</p>
                    </div>
                    <span className="px-2 py-1 bg-green-500/20 text-green-500 rounded-lg text-xs">Active</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-400">Amount:</span>
                    <span className="text-white font-semibold">${investment.amount?.toLocaleString() || 0}</span>
                  </div>
                  <div className="flex justify-between text-sm mt-1">
                    <span className="text-slate-400">ROI:</span>
                    <span className="text-green-500">{investment.roi || investment.dailyROI || 0}%</span>
                  </div>
                </motion.div>
              ))}
            </div>
